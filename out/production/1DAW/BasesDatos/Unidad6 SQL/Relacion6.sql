...
inner join alumno_se_matricula_asignatura am on am.id_alumno = al.id
...
inner join alumno_se_matricula_asignatura al on al.id_asignatura = a.id
inner join alumno alu on alu.id = al.id_alumno
inner join curso_escolar c on c.id = al.id_curso_escolar
where alu.nif like '26902806M';
...
inner join grado g on g.id = a.id_grado
where p.id = (select p.id from profesor p
			inner join asignatura a on a.id_profesor = p.id
            inner join grado g on g.id = a.id_grado
            where g.nombre like 'Grado en Ingeniería Informática (Plan 2015)'
            order by 1 limit 1);
...
inner join alumno_se_matricula_asignatura al on al.id_alumno = a.id
inner join curso_escolar c on c.id = al.id_curso_escolar
